...
- [x] `fixtures/consumer-type-check/consumer.ts` sets `stacked` on the packed artifact.
...
- [x] Those assertions read the *built* scale and the rendered y-axis span, so they fail if
...
- [ ] `updateBarChart` ignores its `stacked` argument, so `cmn-bar-chart`'s `stacked` input is
      construction-time only and silently no-ops when toggled at runtime — the same bug this
      feature fixes for area. Needs its own issue.
